...
echo "agents,avg_cpu_percent,max_cpu_percent,min_cpu_percent" > "$OUTPUT_CPU"
echo "agents,avg_memory_mb,max_memory_mb,min_memory_mb" > "$OUTPUT_MEM"
...
for count in "${AGENT_COUNTS[@]}"; do
    ...
    log_info "CPU: avg=${avg_cpu}% max=${max_cpu}% min=${min_cpu}%"
    log_info "MEM: avg=${avg_mem}MB max=${max_mem}MB min=${min_mem}MB"

    echo "$count,$avg_cpu,$max_cpu,$min_cpu" >> "$OUTPUT_CPU"
    echo "$count,$avg_mem,$max_mem,$min_mem" >> "$OUTPUT_MEM"
