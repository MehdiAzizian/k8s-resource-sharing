...
\begin{itemize}
    ...
    \item \textbf{Comprehensive Evaluation.} Eight automated test scenarios measuring broker scalability (up to 20 clusters), reservation latency (sub-200~ms decisions), concurrent request handling (zero double-bookings), decision accuracy (83--100\%), and advertisement propagation delay (1.4 seconds steady state).
...
\begin{itemize}
    ...
    \item \textbf{Chapter~\ref{ch:implementation}: Implementation.} Details the technology stack, project structure, and implementation of the broker (API server, authentication middleware, reservation controller, decision engine) and agent (resource collector, HTTP transport, reservation polling, Liqo integration).

    \item \textbf{Chapter~\ref{ch:evaluation}: Evaluation.} Presents eight experimental tests conducted on a 96-core server, measuring scalability, bandwidth, agent footprint, reservation latency, concurrent reservation handling, decision accuracy, resource exhaustion, and advertisement freshness.
